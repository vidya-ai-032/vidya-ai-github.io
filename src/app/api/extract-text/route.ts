...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return new Promise<NextResponse>((resolve) => {
      form.parse(request as any, async (err: unknown, fields: Fields, files: Files) => {
        if (err) {
        ...
        try {
          ...
          const extractionResult = await extractTextFromFile(buffer, fileType, fileName);
          
          if (extractionResult.success) {
            console.log("✅ Text extraction successful, length:", extractionResult.text.length);
            resolve(NextResponse.json({
              success: true,
            ...
            resolve(NextResponse.json(
              {
                ...
                userMessage: "Unable to extract text from this file. Please ensure it's a readable PDF, DOCX, or text file."
              },
              { status: 422 }
            ));
          }
          ...
          resolve(NextResponse.json(
            {
              ...
              userMessage: "An error occurred while processing your file. Please try again."
            },
            { status: 500 }
          ));
        }
  ...
  try {
    ...
    if (fileType === "application/pdf" || fileName.toLowerCase().endsWith(".pdf")) {
      ...
      try {
        ...
        if (data && data.text && data.text.trim().length > 0) {
          ...
          if (validateExtractedContent(cleanText)) {
            console.log(`✅ PDF text extraction successful with pdf-parse, length: ${cleanText.length}`);
      ...
      try {
        ...
        if (validateExtractedContent(cleanText)) {
          console.log(`✅ PDF text extraction successful with pdfjs, length: ${cleanText.length}`);
          return { success: true, text: cleanText, method: "pdfjs" };
        } else {
          console.warn("⚠️ PDF parsed with pdfjs but content validation failed");
        }
      } catch (pdfjsError) {
    ...
    // Text file processing
      ...
      if (validateExtractedContent(cleanText)) {
        console.log(`✅ Text file extraction successful, length: ${cleanText.length}`);
